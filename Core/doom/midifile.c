...
static boolean ReadByte(byte *result, FIL *stream)
{
    ...
    // if (f_gets(&c, 1, stream) == NULL)
    if (f_read(stream, &c, sizeof(TCHAR), &read_bytes) != FR_OK)
    {
        fprintf(stderr, "ReadByte: Unexpected end of file\n");
        return false;
    }
                         ...
                         FIL *stream)
{
    ...
    if ((event_type & 0x80) == 0)
    {
        ...
        if (f_lseek(stream, f_tell(stream) - 1) != FR_OK)
        {
            fprintf(stderr, "ReadEvent: Unable to seek in stream\n");
            return false;
        }
...
static boolean ReadFileHeader(midi_file_t *file, FIL *stream)
{
    ...
    if (f_read(stream, &file->header, sizeof(midi_header_t), &records_read) != FR_OK)
    {
        return false;
    }
    ...
    if (!CheckChunkHeader(&file->header.chunk_header, HEADER_CHUNK_ID) || SDL_SwapBE32(file->header.chunk_header.chunk_size) != 6)
    {
        fprintf(stderr, "ReadFileHeader: Invalid MIDI chunk header! "
                        "chunk_size=%i\n",
                (int)SDL_SwapBE32(file->header.chunk_header.chunk_size));
        return false;
    }
...
midi_file_t *MIDI_LoadFile(char *filename)
{
    ...
    FRESULT fres = f_open(&stream, filename, FA_READ);
