...
static char *GetGameName(const char *gamename)
{
    ...
    for (i = 0; i < arrlen(banners); ++i)
    {
        ...
        if (deh_sub != banners[i])
        {
            ...
            while (deh_gamename[0] != '\0' && isspace((uint8_t)deh_gamename[0]))
            {
                memmove(deh_gamename, deh_gamename + 1, gamename_size - 1);
            }

            while (deh_gamename[0] != '\0' && isspace(deh_gamename[

                                                  (uint8_t)(strlen(deh_gamename) - 1)]))
            {
...
{
    return (gamemission == doom2 && W_CheckNumForName("M_RDTHIS") < 0 && W_CheckNumForName("M_EPISOD") < 0 && W_CheckNumForName("M_EPI1") < 0 && W_CheckNumForName("M_EPI2") < 0 && W_CheckNumForName("M_EPI3") < 0 && W_CheckNumForName("WIOSTF") < 0 && W_CheckNumForName("WIOBJ") >= 0);
}
