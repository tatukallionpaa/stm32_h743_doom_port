...
static void OPL_INLINE operator_advance(op_type *op_pt, Bit32s vib)
{
...
static void OPL_INLINE operator_output(op_type *op_pt, Bit32s modulator, Bit32s trem)
{
...
void adlib_write(Bitu idx, Bit8u val)
{
...
#if defined(OPLTYPE_IS_OPL3)
			if ((adlibreg[0x105] & 1) && op[opbase].is_4op_attached)
				break;
#endif
	...
	case ARC_WAVE_SEL + 0x10:
	{
		...
		if ((num < 6) && (base < 22))
		{
			...
			// change waveform
			if (adlibreg[0x105] & 1)
				wave_sel[wselbase] = val & 7; // opl3 mode enabled, all waveforms accessible
			else
				wave_sel[wselbase] = val & 3;
...
void adlib_write_index(Bitu port, Bit8u val)
{
	...
	{
		// possibly second set
		if (((adlibreg[0x105] & 1) != 0) || (opl_index == 5))
			opl_index |= ARC_SECONDSET;
	}
#endif
}
...
void adlib_getsample(Bit16s *sndptr, Bits numsamples)
{
	...
	for (Bits cursmp = 0; cursmp < samples_to_process; cursmp += endsamples)
	{
		...
		if (endsamples > BLOCKBUF_SIZE)
			endsamples = BLOCKBUF_SIZE;

		memset((void *)&outbufl, 0, endsamples * sizeof(Bit32s));
#if defined(OPLTYPE_IS_OPL3)
		// clear second output buffer (opl3 stereo)
		if (adlibreg[0x105] & 1)
			memset((void *)&outbufr, 0, endsamples * sizeof(Bit32s));
#endif
		...
		for (i = 0; i < endsamples; i++)
		{
			...
			if (vibtab_pos / FIXEDPT_LFO >= VIBTAB_SIZE)
		...
		if (adlibreg[ARC_PERC_MODE] & 0x80)
		{
			for (i = 0; i < endsamples; i++)
			{
				...
				if (tremtab_pos / FIXEDPT_LFO >= TREMTAB_SIZE)
					tremtab_pos -= TREMTAB_SIZE * FIXEDPT_LFO;
				trem_lut[i] = trem_table[tremtab_pos / FIXEDPT_LFO];
			}
		}
		else{
			for (i = 0; i < endsamples; i++)
			{
				...
				if (tremtab_pos / FIXEDPT_LFO >= TREMTAB_SIZE)
					tremtab_pos -= TREMTAB_SIZE * FIXEDPT_LFO;
				trem_lut[i] = trem_table[TREMTAB_SIZE + tremtab_pos / FIXEDPT_LFO];
			}
	}
	...
	for (Bits cur_ch = max_channel - 1; cur_ch >= 0; cur_ch--)
	{
		...
		if ((adlibreg[0x105] & 1) && cptr->is_4op_attached)
			continue;
#else
		cptr = &op[cur_ch];
#endif
